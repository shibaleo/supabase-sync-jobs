...
async function getAccessToken(): Promise<string> {
  ...
  const expiresAt = credentials._expires_at ? new Date(credentials._expires_at) : null;
...
export async function listLists(): Promise<TodoList[]> {
  ...
  while (nextLink) {
    const response = await todoRequest<TodoListResponse>("GET", nextLink);
    if (response.value) {
...
export async function updateList(listId: string, displayName: string): Promise<TodoList> {
...
export async function listTasks(params: ListTasksParams): Promise<TodoTask[]> {
  ...
  while (nextLink) {
    const response = await todoRequest<TodoTaskResponse>("GET", nextLink);
    if (response.value) {
...
export async function getTask(listId: string, taskId: string): Promise<TodoTask> {
  return todoRequest<TodoTask>("GET", `/me/todo/lists/${listId}/tasks/${taskId}`);
}
...
export async function createTask(params: CreateTaskParams): Promise<TodoTask> {
  const { listId, title, body, importance, status, dueDateTime, startDateTime, reminderDateTime, categories, isReminderOn } = params;
  ...
  return todoRequest<TodoTask>("POST", `/me/todo/lists/${listId}/tasks`, taskBody);
}
...
export async function updateTask(params: UpdateTaskParams): Promise<TodoTask> {
  const { listId, taskId, title, body, importance, status, dueDateTime, startDateTime, reminderDateTime, categories, isReminderOn } = params;
  ...
  if (reminderDateTime !== undefined) taskBody.reminderDateTime = reminderDateTime;
  if (categories !== undefined) taskBody.categories = categories;
  if (isReminderOn !== undefined) taskBody.isReminderOn = isReminderOn;

  return todoRequest<TodoTask>("PATCH", `/me/todo/lists/${listId}/tasks/${taskId}`, taskBody);
}

export async function completeTask(listId: string, taskId: string): Promise<TodoTask> {
  return updateTask({
    listId,
    taskId,
    status: "completed",
